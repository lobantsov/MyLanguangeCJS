...
bool ClassForMethoodContainer::IsMathOperatorWhitoutIncrement(const Lex &lex) {
    if (lex.value == "+" || lex.value == "-" || lex.value == "*" || lex.value == "/" ||
        lex.value == "+=" || lex.value == "-=" || lex.value == "/=" || lex.value == "*=")
        return true;
    return false;
...
bool ClassForMethoodContainer::IsIncrementa(const Lex& lex) {
    if (lex.value == "++" || lex.value == "--" || lex.value == "**")
        return true;
    return false;
}
