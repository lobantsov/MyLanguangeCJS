...
void Code_execution::EqualExecute(Lex tmp2, Lex tmp1)
{
    for(Lex& op : declarered_variables_->CreatedLexemus)
    {
        ...
        {
            switch(op.dataTypeID)
            {
            case 34:
                op.data_bool = tmp1.data_bool;
                break;
            case 35:
            case 36:
            case 37:
                op.data_number = tmp1.data_number;
                break;
            case 38:
                op.data_string = tmp1.data_string;
                break;
            case 39:
                op.data_char = tmp1.data_char;
                break;
            }
            break;
        }
...
bool Code_execution::CheckLogicSentence(Lex operator_, Lex tmp1, Lex tmp2 )
{
    if (operator_.value == "==")
    {
...
void Code_execution::EaserMyLive()
{
    for(Lex &a : source_string_stack)
    {
        if(a.lexID==LexAnalizator::SingleLexConfig.size() + LexAnalizator::MultiplyLexConfig.size() + 2)
        {
            ...
            {
            case 341:
                if(a.value == "true") a.data_bool = true;
                else if(a.value == "false") a.data_bool =false;
                break;
            case 351:
            case 361:
                a.data_number = stringToNumber(a.value);
                break;
            case 381:
                a.data_string = a.value;
                break;
            case 391: a.data_char = a.value[0];
                break;
            }
        }
    }
...
double Code_execution::stringToNumber(const std::string& str)
{
