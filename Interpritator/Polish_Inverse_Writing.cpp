...
std::vector<Lex> Polish_Inverse_Writing::FormingSourceLine()
{
    ...
    while (iterator < LexAnalizator::FinalLexConfig.size()-1)
    {
        ...
        bool isOperatorBefore = (iterator > 0) && class_for_methood_container_->isOperation(LexAnalizator::FinalLexConfig[iterator - 1].value);
        bool isOperatorAfter = (iterator + 1 < LexAnalizator::FinalLexConfig.size()) &&
            (class_for_methood_container_->isOperation(LexAnalizator::FinalLexConfig[iterator + 1].value)|| LexAnalizator::FinalLexConfig[iterator+1].value==")");
        ...
        if ((lex.lexID == constantID || lex.lexID == variableID) && (isOperatorBefore || isOperatorAfter))
        {
            if(!callBack.empty() and callBack.top() == "for" and (source_string_stack.size()>2 and
                class_for_methood_container_->IsLogicOperator(source_string_stack[source_string_stack.size()-2])) and forOperator)
            {
        ...
        else if(class_for_methood_container_->isOperation(lex.value))
        {
            ...
            if (class_for_methood_container_->IsLogicOperator(lex) or class_for_methood_container_->IsMathOperator(lex) or lex.value == "=")
            {
            ...
            else if(lex.value == "write")
            {
                ...
                if(LexAnalizator::FinalLexConfig[iterator].dataTypeID==381 or LexAnalizator::FinalLexConfig[iterator].dataTypeID == 391)
                {
                   ...
                   Lex tmp;
                    tmp.value = class_for_methood_container_->ExecuteStringSyntaxic(LexAnalizator::FinalLexConfig[iterator].value);
        ...
        else if(lex.value==")")
        {
            ...
            if(mathOperatorPushBack!=0 and class_for_methood_container_->IsLogicOperator(source_string_stack[source_string_stack.size()-1])) mathOperatorPushBack=0;
        ...
        else if(lex.value==";")
        {
            ...
            if(!callBack.empty())
            {
                ...
                {
                    //check i<10
                    if(class_for_methood_container_->IsLogicOperator(source_string_stack[source_string_stack.size()-1]))
                    {
                        source_string_stack.push_back(CreateNewComand("Condition"));
                    }
                    //check and add to tmp stack i++
                    else if(class_for_methood_container_->IsMathOperator(source_string_stack[source_string_stack.size()-1]))
                    {
