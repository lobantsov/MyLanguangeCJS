...
public:
  ...
  int lexLine = -1;
  int dataTypeID = -1;
  int array = -1; //-1 - is not array     0 - is array without new       1 - is
                  //array was init
  std::variant<std::string, bool, char, double, int> data;

  int IndexOfMarkIntoMarkVectorFOR_COMMAND_MOVE = -1;
  int IndexOfMarkIntoMarkVectorFOR_COMMAND_IF = -1;
  ...
  Lex &operator+=(const Lex &other) {
    data = std::visit(
        overloaded{[](auto &&a, auto &&b)
                       -> std::variant<std::string, bool, char, double, int> {
  ...
  bool equals(const Lex &other) const {
    return std::visit(overloaded{[](const auto &a, const auto &b) -> bool {
                        using T = std::decay_t<decltype(a)>;
                        using U = std::decay_t<decltype(b)>;
  ...
  bool operator!=(const Lex &other) const { return !equals(other); }
  ...
  bool operator>(const Lex &other) const { return !(*this <= other); }
